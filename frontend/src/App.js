...
const App = () => {
  ...
  const paginationPrevClickHandler = () => setCurrentPage((prev) => Math.max(prev - 1, 1))
  const paginationNextClickHandler = () => setCurrentPage((prev) => (prev < paginationInfo.totalPages ? prev + 1 : prev))
            ...
            <div data-testid="table" className="table-container">
              {initialFetchDone ? (
                customers.length ? (
                  <>
                    <Greeting name={name} />
                    <Table customers={customers} customerClickHandler={customerClickHandler} />
                    <Pagination
